...
<%
	if(btnval.equalsIgnoreCase("Psearch"))
	{
		...
		try
    	{
	  		...
	  		Connection conn=DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe", "clinic", "clinic");
	  		Statement smt=conn.createStatement();
	  		rs=smt.executeQuery("select * from patient where id='"+t1+"' ");
 %>
	 		<table border=1>
     		<tr>
	 					<th>Patient Id</th>
...
<%		
	...
	if(btnval.equalsIgnoreCase("delete"))
	{
		...
		try
		{
			Class.forName("oracle.jdbc.driver.OracleDriver");
			Connection conn=DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe", "clinic", "clinic");
			PreparedStatement psmt=conn.prepareStatement("delete from patient where id=?");
			psmt.setString(1,t1);
	...
	if(btnval.equalsIgnoreCase("update"))
	{
		...
		try
		{
			Class.forName("oracle.jdbc.driver.OracleDriver");
			Connection conn=DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe", "clinic", "clinic");
			PreparedStatement psmt=conn.prepareStatement("update patient set nm=?,mobile=?,address=?,mail=? where id=?");
			psmt.setString(5,t1);
			psmt.setString(1,t2);
			psmt.setString(2,t3);
			psmt.setString(3,t4);
